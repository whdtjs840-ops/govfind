...
import {
  ...
  mapOntongYouthCandidateToPublicPolicy,
  ontongYouthPaths,
  readJson,
...
async function main() {
  ...
  const previewPath = argValue("preview", ontongYouthPaths.promotionPreview);
  const outPath = argValue("out", ontongYouthPaths.generatorReport);
  ...
  const itemsById = new Map((await loadOntongYouthStagingItems()).map((item) => [item.normalized.sourceItemId, item]));
  const selectedItems = selectedIds.map((id) => itemsById.get(id)).filter(Boolean);
  const mapped = selectedItems.map((item) => {
    const result = mapOntongYouthCandidateToPublicPolicy(item);
    return {
  ...
  const report = {
    ...
    recommendedNextAction: validation.valid ? "run_ontong_youth_apply_dry_run" : "need_mapping_fix"
  };
  ...
  console.log("OntongYouth promotion generator dry-run");
