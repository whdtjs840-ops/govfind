...
import { ontongYouthAdapter, OntongYouthApiError } from "./importers/ontong-youth-adapter.mjs";
import {
  GOVFIND_CATEGORIES,
  GOVFIND_REGIONS,
  ONTONG_YOUTH_SOURCE_NAME,
  normalizeComparableText
} from "./importers/ontong-youth-normalizers.mjs";
...
function isGenericOfficialUrl(value = "") {
  ...
  if (!url) return false;
  if (url === "https://www.youthcenter.go.kr") return true;
  if (/^https:\/\/www\.youthcenter\.go\.kr\/?$/.test(url)) return true;
  return /^https:\/\/www\.youthcenter\.go\.kr\/go\/ythip\/?$/.test(url);
}
...
function buildExistingPolicyIndex(items) {
  ...
  for (const policy of items) {
    if (policy.sourceName && policy.sourceItemId) {
      bySourceItemId.set(`${policy.sourceName}::${policy.sourceItemId}`, policy);
    }
...
function buildStagingItem({ rawIndex, listItem, detail, normalized, existingIndex }) {
  ...
  return {
    ...
    rawRef: { sourceName: ONTONG_YOUTH_SOURCE_NAME, sourceItemId: normalized.sourceItemId, rawIndex },
    raw: { listItem, detail }
  };
}
...
function buildReport({ rawPayload, stagingPayload, savedPaths = null, status = "success", error = null }) {
  ...
  return {
    ...
    recommendedNextAction:
      status === "error"
        ? "stop_and_review"
        : items.length === classifiedTotal && !Object.keys(mappingGaps).includes("category") && uniqueNewCandidateCount > 0
          ? "ready_for_ontong_youth_candidate_dry_run"
          : Object.keys(mappingGaps).includes("category")
            ? "need_mapping_fix"
            : uniqueNewCandidateCount > 0
              ? "need_more_discovery"
              : "move_to_bizinfo_or_kstartup",
...
async function saveArtifacts({ outDir, rawPayload, stagingPayload, report }) {
  ...
  const rawPath = resolve(outDir, "raw", `${ONTONG_YOUTH_SOURCE_NAME}-${stamp}.raw.json`);
  const stagingPath = resolve(outDir, "staging", `${ONTONG_YOUTH_SOURCE_NAME}-${stamp}.staging.json`);
  const reportPath = resolve(outDir, "reports", `${ONTONG_YOUTH_SOURCE_NAME}-${stamp}.report.json`);
  const fixedReportPath = resolve("data/staging/ontong-youth/dry-run-report.json");
...
function printReport(report) {
  console.log("OntongYouth dry-run importer");
...
export async function runOntongYouthDryRun({ page = 1, limit = 20, save = false, outDir = "data/imports/ontong-youth" } = {}) {
  const existingIndex = buildExistingPolicyIndex(getPublicPolicies(policies));

  if (!process.env.GOVFIND_ONTONG_YOUTH_API_KEY && !process.env.GOVFIND_YOUTH_API_KEY) {
    const rawPayload = {
      meta: {
        sourceName: ONTONG_YOUTH_SOURCE_NAME,
        endpointName: "getPlcy",
        requestParams: { pageNum: page, pageSize: limit, rtnType: "json", apiKeyNm: "[REDACTED]" }
      },
      items: []
    };
    const stagingPayload = { sourceName: ONTONG_YOUTH_SOURCE_NAME, fetchedAt: new Date().toISOString(), items: [] };
    const report = buildReport({
      rawPayload,
      stagingPayload,
      status: "error",
      error: { kind: "missing_api_key", message: "GOVFIND_ONTONG_YOUTH_API_KEY is not set" }
    });
  ...
  try {
    const listPayload = await ontongYouthAdapter.fetchPage({ page, limit });
    const rawItems = [];
    for (let index = 0; index < listPayload.items.length; index += 1) {
      const listItem = listPayload.items[index];
      const detail = await ontongYouthAdapter.fetchDetail(listItem);
      rawItems.push({ listItem, detail, page, indexOnPage: index });
    }
    const rawPayload = {
      meta: {
        ...listPayload.meta,
        adapterName: ONTONG_YOUTH_SOURCE_NAME,
        requestParams: listPayload.meta.requestParams
      },
      items: rawItems
    };
    const stagingItems = rawItems.map((entry, rawIndex) => {
      const normalized = ontongYouthAdapter.normalizeItem(entry.listItem, entry.detail);
    ...
    const report = buildReport({
      ...
      status: "error",
      error:
        error instanceof OntongYouthApiError
          ? error
...
async function main() {
  ...
  const page = Number(argValue("page", process.env.GOVFIND_ONTONG_YOUTH_PAGE ?? 1));
  const limit = Number(argValue("limit", argValue("per-page", process.env.GOVFIND_ONTONG_YOUTH_PER_PAGE ?? 20)));
  const save = hasFlag("save");
  const outDir = argValue("out-dir", "data/imports/ontong-youth");
  const { report } = await runOntongYouthDryRun({ page, limit, save, outDir });
  printReport(report);
