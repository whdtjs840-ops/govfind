...
import { argValue, ontongYouthPaths, readJson } from "./importers/ontong-youth-promotion-utils.mjs";
...
function assertApplyReady(dryRunReport, limit) {
  ...
  if (failures.length) {
    throw new Error(`OntongYouth apply preflight failed: ${failures.join("; ")}`);
  }
}
...
function assertCandidateSafety(policies, limit) {
  ...
  policies.forEach((policy, index) => {
    ...
    if (!policy.slug?.startsWith("ontong-youth-")) failures.push(`${label}: slug does not use ontong-youth prefix`);
  ...
  if (failures.length) {
    throw new Error(`OntongYouth candidate safety failed:\n${failures.join("\n")}`);
  }
}
...
async function main() {
  const limit = Number.parseInt(argValue("limit", "300"), 10);
  ...
  const applyReportPath = `data/staging/ontong-youth/apply-report-ontong-youth-batch-${batch}.json`;
  const sourceText = await readFile(policiesPath, "utf8");
  const blockName = batch === 1 ? "ontongYouthPromotionPolicies" : `ontongYouthPromotionPoliciesBatch${batch}`;

  if (sourceText.includes(`const ${blockName}`)) {
    throw new Error(`${blockName} block already exists. Refusing to apply twice.`);
  }

  const dryRunReport = await readJson(ontongYouthPaths.applyDryRunReport);
  assertApplyReady(dryRunReport, limit);

  const generatedPreview = await readJson(ontongYouthPaths.generatedPreview);
  ...
  const report = {
    ...
    buildResult: "pending",
    rollbackNote: "Revert only the ontongYouthPromotionPolicies block and its policies.push call from src/data/policies.ts, then rerun validation/test/build.",
    applyReadiness: "applied_pending_validation"
  };

  await writeJson(applyReportPath, report);

  console.log("OntongYouth apply completed");
