...
import { bokjiroCentralAdapter, BokjiroCentralApiError } from "./importers/bokjiro-central-adapter.mjs";
import {
  BOKJIRO_CENTRAL_SOURCE_NAME,
  GOVFIND_CATEGORIES,
  GOVFIND_REGIONS,
  normalizeComparableText
} from "./importers/bokjiro-central-normalizers.mjs";
...
function duplicateCandidates(normalized, existingIndex) {
  ...
  if (supportText) {
    for (const policy of existingIndex.all) {
      ...
      if (policyText && titleSimilarity(supportText, policyText) >= 0.72 && titleSimilarity(normalized.title, policy.title) >= 0.5) {
        candidates.push({
          rule: "title + supportContent similarity",
          strength: "medium",
          matchedSlug: policy.slug,
          matchedTitle: policy.title,
          value: titleSimilarity(supportText, policyText).toFixed(2)
        });
        break;
      }
    }
...
function buildStagingItem({ rawIndex, listItem, detail, normalized, existingIndex }) {
  ...
  return {
    ...
    rawRef: { sourceName: BOKJIRO_CENTRAL_SOURCE_NAME, sourceItemId: normalized.sourceItemId, rawIndex },
    raw: { listItem, detail }
  };
}
...
function buildReport({ rawPayload, stagingPayload, savedPaths = null, status = "success", error = null }) {
  ...
  const publishableTotal = readyCount + publishableWithWarningCount;
...
async function saveArtifacts({ outDir, rawPayload, stagingPayload, report }) {
  ...
  const rawPath = resolve(outDir, "raw", `${BOKJIRO_CENTRAL_SOURCE_NAME}-${stamp}.raw.json`);
  const stagingPath = resolve(outDir, "staging", `${BOKJIRO_CENTRAL_SOURCE_NAME}-${stamp}.staging.json`);
  const reportPath = resolve(outDir, "reports", `${BOKJIRO_CENTRAL_SOURCE_NAME}-${stamp}.report.json`);
  const fixedReportPath = resolve("data/staging/bokjiro-central/dry-run-report.json");
...
function printReport(report) {
  console.log("Bokjiro central dry-run importer");
...
export async function runBokjiroCentralDryRun({ page = 1, limit = 20, save = false, outDir = "data/imports/bokjiro-central" } = {}) {
  const existingIndex = buildExistingPolicyIndex(policies);

  if (!process.env.GOVFIND_BOKJIRO_CENTRAL_API_KEY && !process.env.GOVFIND_WELFARE_API_KEY) {
    const rawPayload = {
      meta: {
        sourceName: BOKJIRO_CENTRAL_SOURCE_NAME,
    ...
    const stagingPayload = { sourceName: BOKJIRO_CENTRAL_SOURCE_NAME, fetchedAt: new Date().toISOString(), items: [] };
    const report = buildReport({
      rawPayload,
      stagingPayload,
      status: "error",
      error: { kind: "missing_api_key", message: "GOVFIND_BOKJIRO_CENTRAL_API_KEY is not set" }
    });
  ...
  try {
    const listPayload = await bokjiroCentralAdapter.fetchPage({ page, limit });
    const rawItems = [];
    for (let index = 0; index < listPayload.items.length; index += 1) {
      const listItem = listPayload.items[index];
      const detail = await bokjiroCentralAdapter.fetchDetail(listItem);
      rawItems.push({ listItem, detail, page, indexOnPage: index });
    }
    const rawPayload = {
      meta: {
        ...listPayload.meta,
        adapterName: BOKJIRO_CENTRAL_SOURCE_NAME,
        requestParams: listPayload.meta.requestParams
      },
      items: rawItems
    };
    const stagingItems = rawItems.map((entry, rawIndex) => {
      const normalized = bokjiroCentralAdapter.normalizeItem(entry.listItem, entry.detail);
    ...
    const report = buildReport({
      ...
      status: "error",
      error:
        error instanceof BokjiroCentralApiError
          ? error
...
async function main() {
  ...
  const page = Number(argValue("page", process.env.GOVFIND_BOKJIRO_CENTRAL_PAGE ?? 1));
  const limit = Number(argValue("limit", argValue("per-page", process.env.GOVFIND_BOKJIRO_CENTRAL_PER_PAGE ?? 20)));
  const save = hasFlag("save");
  const outDir = argValue("out-dir", "data/imports/bokjiro-central");
  const { report } = await runBokjiroCentralDryRun({ page, limit, save, outDir });
  printReport(report);
