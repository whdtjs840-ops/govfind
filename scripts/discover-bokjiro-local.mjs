...
import { runBokjiroLocalDryRun } from "./import-bokjiro-local-dry-run.mjs";
...
async function listReportFiles(directory = "data/imports/bokjiro-local/reports") {
...
async function loadReports() {
  ...
  for (const file of await listReportFiles()) {
    try {
      ...
      if (report.sourceName === "bokjiro-local") reports.push({ ...report, reportFilePath: file });
    } catch {
...
function successfulPageSummary(report) {
  return {
    ...
    uniqueNewCandidateCount: Number(report.readyCount ?? 0) + Number(report.publishableWithWarningCount ?? 0),
...
function buildAggregateReport({ allReports, pagesTried, pagesSucceeded, pagesFailed, invalidPages, apiQuotaError, currentLimit }) {
  ...
  const recommendedNextAction =
    apiQuotaError
      ? "stop_and_review"
      : uniqueNewCandidateCount >= 30 && !categoryGap
...
async function main() {
  ...
  const limit = Number(argValue("limit", process.env.GOVFIND_BOKJIRO_LOCAL_PER_PAGE ?? 50));
  ...
  const discoveryReportPath = resolve(argValue("report", "data/staging/bokjiro-local/bokjiro-local-discovery-report.json"));
  ...
  for (const targetPage of pages) {
    ...
    const { report } = await runBokjiroLocalDryRun({ page: targetPage, limit, save, outDir });
    ...
    pagesSucceeded.push(targetPage);
    console.log(
      `page ${targetPage}: status=${report.status} rawCount=${report.rawCount} normalizedCount=${report.normalizedCount} readyCount=${report.readyCount} publishableWithWarningCount=${report.publishableWithWarningCount} needsReviewCount=${report.needsReviewCount} duplicateCount=${report.duplicateCount} incompleteCount=${report.incompleteCount} uniqueNewCandidateCount=${report.readyCount + report.publishableWithWarningCount}`
    );
  }
