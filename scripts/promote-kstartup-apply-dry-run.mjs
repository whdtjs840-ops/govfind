...
async function main() {
  ...
  const generatedPath = argValue("generated", kstartupPaths.generatedPreview);
  const outPath = argValue("out", kstartupPaths.applyDryRunReport);
  const mergedPath = argValue("merged", kstartupPaths.mergedTemp);
  ...
  const selectedItemSummaries = selectedPolicies.map((policy) => ({
    sourceItemId: policy.slug?.replace(/^kstartup-/, "") ?? null,
    title: policy.title,
    slug: policy.slug,
  ...
  const candidateReport = await readJson(kstartupPaths.promotionPreview).catch(() => null);
  const report = {
    ...
    applyReadiness,
    recommendedNextAction: applyReadiness === "ready_for_apply" ? "ready_for_kstartup_large_apply" : "need_mapping_fix"
  };
  ...
  console.log("K-Startup apply dry-run");
