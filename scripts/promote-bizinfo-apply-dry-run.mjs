...
async function main() {
  ...
  const outPath = argValue("out", bizinfoPaths.applyDryRunReport);
  const mergedPath = argValue("merged", bizinfoPaths.mergedTemp);
  ...
  const selectedItemSummaries = selectedPolicies.map((policy) => ({
    sourceItemId: policy.slug?.replace(/^bizinfo-/, "") ?? null,
    title: policy.title,
    slug: policy.slug,
  ...
  const candidateReport = await readJson(bizinfoPaths.promotionPreview).catch(() => null);
  const report = {
    ...
    applyReadiness,
    recommendedNextAction: applyReadiness === "ready_for_apply" ? "ready_for_bizinfo_large_apply" : "need_mapping_fix"
  };
  ...
  console.log("Bizinfo apply dry-run");
