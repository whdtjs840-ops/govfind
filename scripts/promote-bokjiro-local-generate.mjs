...
import {
  ...
  mapBokjiroLocalCandidateToPublicPolicy,
  readJson,
...
async function main() {
  ...
  const previewPath = argValue("preview", bokjiroLocalPaths.promotionPreview);
  const outPath = argValue("out", bokjiroLocalPaths.generatorReport);
  ...
  const itemsById = new Map((await loadBokjiroLocalStagingItems()).map((item) => [item.normalized.sourceItemId, item]));
  const selectedItems = selectedIds.map((id) => itemsById.get(id)).filter(Boolean);
  const mapped = selectedItems.map((item) => {
    const result = mapBokjiroLocalCandidateToPublicPolicy(item);
    return {
  ...
  const report = {
    ...
    recommendedNextAction: validation.valid ? "run_bokjiro_local_apply_dry_run" : "need_mapping_fix"
  };
  ...
  console.log("Bokjiro local promotion generator dry-run");
