...
import { argValue, kstartupPaths, readJson } from "./importers/kstartup-promotion-utils.mjs";
...
function assertApplyReady(dryRunReport, limit) {
  ...
  if (failures.length) {
    throw new Error(`K-Startup apply preflight failed: ${failures.join("; ")}`);
  }
}
...
function assertCandidateSafety(policies, limit) {
  ...
  policies.forEach((policy, index) => {
    ...
    if (!policy.slug?.startsWith("kstartup-")) failures.push(`${label}: slug does not use kstartup prefix`);
    ...
    if (policy.category === "복지") failures.push(`${label}: startup support item mapped to welfare`);
  ...
  if (failures.length) {
    throw new Error(`K-Startup candidate safety failed:\n${failures.join("\n")}`);
  }
}
...
async function main() {
  ...
  const applyReportPath = `data/staging/kstartup/apply-report-kstartup-batch-${batch}.json`;
  const sourceText = await readFile(policiesPath, "utf8");
  const blockName = batch === 1 ? "kstartupPromotionPolicies" : `kstartupPromotionPoliciesBatch${batch}`;

  if (sourceText.includes(`const ${blockName}`)) {
    throw new Error(`${blockName} block already exists. Refusing to apply twice.`);
  }

  const dryRunReport = await readJson(kstartupPaths.applyDryRunReport);
  assertApplyReady(dryRunReport, limit);

  const generatedPreview = await readJson(kstartupPaths.generatedPreview);
  ...
  const report = {
    ...
    buildResult: "pending",
    rollbackNote: "Revert only the kstartupPromotionPolicies block and its policies.push call from src/data/policies.ts, then rerun validation/test/build.",
    applyReadiness: "applied_pending_validation"
  };

  await writeJson(applyReportPath, report);

  console.log("K-Startup apply completed");
