...
async function main() {
  ...
  const generatedPath = argValue("generated", ontongYouthPaths.generatedPreview);
  const outPath = argValue("out", ontongYouthPaths.applyDryRunReport);
  const mergedPath = argValue("merged", ontongYouthPaths.mergedTemp);
  ...
  const selectedItemSummaries = selectedPolicies.map((policy) => ({
    sourceItemId: policy.officialUrl?.match(/plcyNo=([^&]+)/)?.[1] ?? null,
    title: policy.title,
    slug: policy.slug,
  ...
  const candidateReport = await readJson(ontongYouthPaths.promotionPreview).catch(() => null);
  const report = {
    ...
    statusCountPreview,
    sourceCountPreview,
    sampleSelectedItemUrls: selectedItemSummaries.slice(0, 20).map((item) => item.previewUrl),
    selectedItemSummaries,
    applyReadiness,
    recommendedNextAction: applyReadiness === "ready_for_apply" ? "ready_for_ontong_youth_apply" : "need_mapping_fix"
  };
  ...
  console.log("OntongYouth apply dry-run");
