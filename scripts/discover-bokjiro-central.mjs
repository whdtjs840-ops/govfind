...
import { runBokjiroCentralDryRun } from "./import-bokjiro-central-dry-run.mjs";
...
async function listReportFiles(directory = "data/imports/bokjiro-central/reports") {
...
async function loadReports() {
  ...
  for (const file of await listReportFiles()) {
    try {
      ...
      if (report.sourceName === "bokjiro-central") reports.push({ ...report, reportFilePath: file });
    } catch {
      // Ignore malformed runtime artifacts; validation is based on well-formed reports.
    }
  }
  return reports;
}
...
function uniqueItemKey(item) {
  return `${item.sourceItemId ?? ""}::${item.title ?? ""}`;
}
...
function buildAggregateReport({ allReports, pagesTried, pagesSucceeded, pagesFailed, invalidPages, apiQuotaError, currentLimit }) {
  const successfulReports = allReports.filter((report) => report.status === "success");
  ...
  const remainingCandidateCount = publishableCandidates.size || readyCount + publishableWithWarningCount;
  const warningCount = Object.values(mergeGapCounts(successfulReports)).reduce((sum, count) => sum + Number(count), 0);
  const blockerCount = needsReviewCount + duplicateCount + incompleteCount;
  const recommendedNextAction =
    apiQuotaError
      ? "stop_and_review"
      : remainingCandidateCount >= 50
        ? "ready_for_bokjiro_candidate_dry_run"
        : remainingCandidateCount > 0 && Object.keys(mergeGapCounts(successfulReports)).includes("category")
          ? "need_mapping_fix"
          : duplicateCount >= normalizedCount * 0.75
            ? "bokjiro_use_as_update_source"
            : remainingCandidateCount < 5
              ? "move_to_next_data_source"
              : "need_mapping_fix";
  ...
  return {
    ...
    duplicateCount,
    incompleteCount,
    uniqueNewCandidateCount: readyCount + publishableWithWarningCount,
    duplicateCandidatesCount,
    mappingGaps: mergeGapCounts(successfulReports),
    blockerCount,
    warningCount,
...
async function main() {
  ...
  const limit = Number(argValue("limit", process.env.GOVFIND_BOKJIRO_CENTRAL_PER_PAGE ?? 50));
  ...
  const discoveryReportPath = resolve(argValue("report", "data/staging/bokjiro-central/discovery-report.json"));
  ...
  for (const targetPage of pages) {
    ...
    const { report } = await runBokjiroCentralDryRun({ page: targetPage, limit, save, outDir });
    ...
    pagesSucceeded.push(targetPage);
    console.log(
      `page ${targetPage}: status=${report.status} rawCount=${report.rawCount} normalizedCount=${report.normalizedCount} readyCount=${report.readyCount} publishableWithWarningCount=${report.publishableWithWarningCount} needsReviewCount=${report.needsReviewCount} duplicateCount=${report.duplicateCount} incompleteCount=${report.incompleteCount} duplicateCandidatesCount=${report.duplicateCandidatesCount}`
    );
  }
