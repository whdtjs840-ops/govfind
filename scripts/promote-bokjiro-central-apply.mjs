...
import { argValue, bokjiroPaths, readJson } from "./importers/bokjiro-promotion-utils.mjs";
...
function assertApplyReady(dryRunReport, limit) {
  ...
  if (failures.length) {
    throw new Error(`Bokjiro central apply preflight failed: ${failures.join("; ")}`);
  }
}
...
function assertCandidateSafety(policies, limit) {
  ...
  policies.forEach((policy, index) => {
    ...
    if (!policy.slug?.startsWith("bokjiro-central-")) failures.push(`${label}: slug does not use bokjiro-central prefix`);
  ...
  if (failures.length) {
    throw new Error(`Bokjiro central candidate safety failed:\n${failures.join("\n")}`);
  }
}
...
async function main() {
  ...
  const generatedPreview = await readJson(bokjiroPaths.generatedPreview);
  ...
  const report = {
    ...
    sampleQaUrls: {
      publishableWithWarning: selectedItemSummaries.slice(0, 10).map((item) => item.url),
      unknownDate: selectedItemSummaries.filter((item) => item.dateConfidence === "unknown").slice(0, 10).map((item) => item.url),
      unknownStatus: selectedItemSummaries.filter((item) => item.statusConfidence === "unknown").slice(0, 10).map((item) => item.url),
      unknownRegion: selectedItemSummaries.filter((item) => item.region === null).slice(0, 10).map((item) => item.url),
      requiresOfficialConfirmation: selectedItemSummaries.filter((item) => item.requiresOfficialConfirmation).slice(0, 10).map((item) => item.url)
    },
    validationResults: {
    ...
    buildResult: "pending",
    rollbackNote: "Revert only the bokjiroCentralPromotionPolicies block and its policies.push call from src/data/policies.ts, then rerun validation/test/build.",
    applyReadiness: "applied_pending_validation"
  };

  await writeJson(applyReportPath, report);

  console.log("Bokjiro central apply completed");
