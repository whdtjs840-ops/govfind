...
export const bokjiroLocalPaths = {
  promotionPreview: "data/staging/bokjiro-local/promotion-preview.json",
  generatorReport: "data/staging/bokjiro-local/promotion-generator-report.json",
  generatedPreview: "data/staging/bokjiro-local/promotion-generated-preview.json",
  applyDryRunReport: "data/staging/bokjiro-local/bokjiro-local-candidate-dry-run-report.json",
  mergedTemp: "data/staging/bokjiro-local/apply-dry-run-merged.tmp.json"
};
...
function buildExistingPolicyIndex(items = getPublicPolicies(policies)) {
  ...
  for (const policy of items) {
    ...
    for (const url of [policy.officialUrl, policy.officialSourceUrl].filter(Boolean)) {
      ...
      if (bokjiroId && /wlfareInfoReldBztpCd=02|지자체|local-/i.test(`${url} ${policy.source ?? ""} ${policy.slug ?? ""}`)) {
        bySourceItemId.set(`bokjiro-local::${bokjiroId}`, policy);
      }
    }
...
export async function loadBokjiroLocalStagingItems() {
  const stagingFiles = await listJsonFiles("data/imports/bokjiro-local/staging", ".staging.json");
  const bySourceItemId = new Map();
...
export function summarizeBokjiroLocalStagingItem(item) {
  const normalized = item.normalized ?? {};
  return {
...
export function mapBokjiroLocalCandidateToPublicPolicy(item) {
  const normalized = item.normalized ?? {};
  const classification = classificationOf(item);
  const issues = [];
  ...
  return {
    ...
    publicPolicyPreview: {
      ...
      deadline,
      dday: dateUnknown ? "확인필요" : status === "상시" ? "상시" : "확인필요",
      status,
      ...
      documents: ["공식 공고 확인"],
      apply: normalized.applicationMethod || "복지로 또는 해당 지자체 공식 안내에서 신청 방법을 확인하세요.",
      ...
      matchReasons: ["복지로 지자체복지 공식 API 기준", "신청기간과 자격은 공식 공고 확인 필요"],
      faq: [
        {
          q: "GovFind에서 신청 가능 여부를 확정하나요?",
          a: "아니요. 최종 자격과 신청 가능 여부는 복지로 또는 해당 지자체 공식 안내에서 확인해야 합니다."
        }
      ],
      apiDetails: {
        target: audience,
