...
export const ontongYouthPaths = {
  promotionPreview: "data/staging/ontong-youth/promotion-preview.json",
  generatorReport: "data/staging/ontong-youth/promotion-generator-report.json",
  generatedPreview: "data/staging/ontong-youth/promotion-generated-preview.json",
  applyDryRunReport: "data/staging/ontong-youth/ontong-youth-candidate-dry-run-report.json",
  mergedTemp: "data/staging/ontong-youth/apply-dry-run-merged.tmp.json"
};
...
export async function loadOntongYouthStagingItems() {
  const stagingFiles = await listJsonFiles("data/imports/ontong-youth/staging", ".staging.json");
  const bySourceItemId = new Map();
...
export function selectOntongYouthCandidatesFromItems({ items, limit = 300, publicPolicies = getAppliedPoliciesForConflictCheck(policies) }) {
...
export function summarizeOntongYouthStagingItem(item) {
  const normalized = item.normalized ?? {};
  return {
...
export function mapOntongYouthCandidateToPublicPolicy(item) {
  const normalized = item.normalized ?? {};
  const classification = classificationOf(item);
  const issues = [];
  ...
  const status = "확인필요";
  const statusLabel = "공식 공고 확인";
  const deadline = "공식 공고 확인";
  const officialConfirmation = true;
  ...
  const publicPolicyPreview = {
    ...
    publishPolicy,
    lifeStage: "청년",
    targetGroup: (normalized.targetGroups ?? []).join(", ") || "청년",
    income: "공식 공고 확인",
    ...
    documents: ["공식 공고 확인"],
    apply: normalized.applicationMethod || "온통청년 또는 해당 기관의 공식 안내에서 신청 방법을 확인하세요.",
    ...
    matchReasons: ["온통청년 공식 API 기준", "신청기간과 자격은 공식 공고 확인 필요"],
    faq: [
      {
  ...
  if (!publicStatuses.has(publicPolicyPreview.status)) {
    return {
      bucket: "rejected",
      issues: [`unsupported status: ${publicPolicyPreview.status}`],
      publicPolicyPreview: null,
      recommendedAction: "Do not promote until status mapping is fixed."
    };
  }

  return {
    bucket: "promotable_with_safe_mapping",
    issues: [],
    publicPolicyPreview,
