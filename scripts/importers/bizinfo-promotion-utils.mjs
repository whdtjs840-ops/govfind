...
export const bizinfoPaths = {
  promotionPreview: "data/staging/bizinfo/promotion-preview.json",
  generatorReport: "data/staging/bizinfo/promotion-generator-report.json",
  generatedPreview: "data/staging/bizinfo/promotion-generated-preview.json",
  applyDryRunReport: "data/staging/bizinfo/bizinfo-candidate-dry-run-report.json",
  mergedTemp: "data/staging/bizinfo/apply-dry-run-merged.tmp.json"
};
...
export async function loadBizinfoStagingItems() {
  const stagingFiles = await listJsonFiles("data/imports/bizinfo/staging", ".staging.json");
  const bySourceItemId = new Map();
...
export function isExpiredBizinfoCandidate(item, today = getPolicyToday()) {
...
export function selectBizinfoCandidatesFromItems({ items, limit = 500, publicPolicies = getAppliedPoliciesForConflictCheck(policies) }) {
  ...
  for (const item of items) {
    ...
    if (isExpiredBizinfoCandidate(item)) {
...
export function summarizeBizinfoStagingItem(item) {
  const normalized = item.normalized ?? {};
  return {
...
export function mapBizinfoCandidateToPublicPolicy(item) {
  const normalized = item.normalized ?? {};
  const classification = classificationOf(item);
  const issues = [];
  ...
  if (isExpiredBizinfoCandidate(item)) issues.push("expired candidate");
  ...
  return {
    ...
    publicPolicyPreview: {
      ...
      publishPolicy,
      lifeStage: "기업",
      targetGroup: (normalized.targetGroups ?? []).join(", ") || "기업",
      income: "공식 공고 확인",
      ...
      documents: ["공식 공고 확인"],
      apply: normalized.applicationMethod || "기업마당 또는 해당 기관 공식 공고에서 신청 방법을 확인하세요.",
      ...
      matchReasons: ["기업마당 공식 API 기준", "신청기간과 자격은 공식 공고 확인 필요"],
      faq: [
        {
          q: "GovFind에서 신청 가능 여부를 확정하나요?",
          a: "아닙니다. 최종 자격과 신청 가능 여부는 기업마당 또는 해당 기관의 공식 공고에서 확인해야 합니다."
        }
      ],
      apiDetails: {
        target: audience,
