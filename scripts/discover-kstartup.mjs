...
import { runKstartupDryRun } from "./import-kstartup-dry-run.mjs";
...
async function listReportFiles(directory = "data/imports/kstartup/reports") {
...
async function loadReports() {
  ...
  for (const file of await listReportFiles()) {
    try {
      ...
      if (report.sourceName === "kstartup") reports.push({ ...report, reportFilePath: file });
    } catch {
...
async function main() {
  ...
  const limit = Number(argValue("limit", process.env.GOVFIND_KSTARTUP_PER_PAGE ?? 100));
  ...
  const discoveryReportPath = resolve(argValue("report", "data/staging/kstartup/kstartup-discovery-and-candidate-report.json"));
  ...
  for (const targetPage of pages) {
    ...
    const { report } = await runKstartupDryRun({ page: targetPage, limit, save, outDir });
