...
import {
  ...
  mapKstartupCandidateToPublicPolicy,
  readJson,
...
async function main() {
  ...
  const previewPath = argValue("preview", kstartupPaths.promotionPreview);
  const outPath = argValue("out", kstartupPaths.generatorReport);
  ...
  const itemsById = new Map((await loadKstartupStagingItems()).map((item) => [item.normalized.sourceItemId, item]));
  const selectedItems = selectedIds.map((id) => itemsById.get(id)).filter(Boolean);
  const mapped = selectedItems.map((item) => {
    const result = mapKstartupCandidateToPublicPolicy(item);
    return {
  ...
  const report = {
    ...
    recommendedNextAction: validation.valid ? "run_kstartup_apply_dry_run" : "need_mapping_fix"
  };
  ...
  console.log("K-Startup promotion generator dry-run");
