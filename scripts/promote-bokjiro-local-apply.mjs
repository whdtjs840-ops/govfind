...
import { argValue, bokjiroLocalPaths, readJson } from "./importers/bokjiro-local-promotion-utils.mjs";
...
function assertApplyReady(dryRunReport, limit) {
  ...
  if (failures.length) {
    throw new Error(`Bokjiro local apply preflight failed: ${failures.join("; ")}`);
  }
}
...
function assertCandidateSafety(policies, limit) {
  ...
  policies.forEach((policy, index) => {
    ...
    if (!policy.slug?.startsWith("bokjiro-local-")) failures.push(`${label}: slug does not use bokjiro-local prefix`);
    ...
    if (policy.status !== "확인필요") failures.push(`${label}: status must remain 확인필요`);
    if (policy.statusLabel !== "공식 공고 확인") failures.push(`${label}: statusLabel must remain 공식 공고 확인`);
    if (policy.statusConfidence !== "unknown") failures.push(`${label}: statusConfidence must remain unknown`);
    if (policy.startDate !== null) failures.push(`${label}: startDate must remain null`);
    if (policy.endDate !== null) failures.push(`${label}: endDate must remain null`);
    if (policy.dateConfidence !== "unknown") failures.push(`${label}: dateConfidence must remain unknown`);
    if (policy.applicationPeriodLabel !== "공식 공고 확인") failures.push(`${label}: applicationPeriodLabel must remain 공식 공고 확인`);
    if (policy.requiresOfficialConfirmation !== true) failures.push(`${label}: requiresOfficialConfirmation must be true`);
    if (!Array.isArray(policy.warnings) || !policy.warnings.length) failures.push(`${label}: warnings must be preserved`);
    if (policy.publishPolicy?.canPublish !== true) failures.push(`${label}: publishPolicy.canPublish is not true`);
    if (policy.publishPolicy?.includeInStatusFilters !== false) failures.push(`${label}: unknown status item would enter status filters`);
  ...
  if (failures.length) {
    throw new Error(`Bokjiro local candidate safety failed:\n${failures.join("\n")}`);
  }
}
...
async function main() {
  const limit = Number.parseInt(argValue("limit", "70"), 10);
  ...
  const applyReportPath = `data/staging/bokjiro-local/apply-report-bokjiro-local-batch-${batch}.json`;
  const sourceText = await readFile(policiesPath, "utf8");
  const blockName = batch === 1 ? "bokjiroLocalPromotionPolicies" : `bokjiroLocalPromotionPoliciesBatch${batch}`;

  if (sourceText.includes(`const ${blockName}`)) {
    throw new Error(`${blockName} block already exists. Refusing to apply twice.`);
  }

  const dryRunReport = await readJson(bokjiroLocalPaths.applyDryRunReport);
  assertApplyReady(dryRunReport, limit);

  const generatedPreview = await readJson(bokjiroLocalPaths.generatedPreview);
  const selectedPolicies = (generatedPreview.policies ?? []).slice(0, limit);
  assertCandidateSafety(selectedPolicies, limit);

  const insertBefore = "policies.push(...localWelfareApiPolicies);";
  ...
  const report = {
    ...
    sampleQaUrls: {
      publishableWithWarning: selectedItemSummaries.slice(0, 20).map((item) => item.url),
      unknownDate: selectedItemSummaries.filter((item) => item.dateConfidence === "unknown").slice(0, 20).map((item) => item.url),
      unknownStatus: selectedItemSummaries.filter((item) => item.statusConfidence === "unknown").slice(0, 20).map((item) => item.url),
      unknownRegion: selectedItemSummaries.filter((item) => item.region === null).slice(0, 20).map((item) => item.url),
      requiresOfficialConfirmation: selectedItemSummaries.filter((item) => item.requiresOfficialConfirmation).slice(0, 20).map((item) => item.url)
    },
    validationResults: {
    ...
    buildResult: "pending",
    rollbackNote: "Revert only the bokjiroLocalPromotionPolicies block and its policies.push call from src/data/policies.ts, then rerun validation/test/build.",
    applyReadiness: "applied_pending_validation"
  };

  await writeJson(applyReportPath, report);

  console.log("Bokjiro local apply completed");
