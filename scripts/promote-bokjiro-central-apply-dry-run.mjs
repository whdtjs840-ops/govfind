...
async function main() {
  ...
  const outPath = argValue("out", bokjiroPaths.applyDryRunReport);
  const mergedPath = argValue("merged", bokjiroPaths.mergedTemp);
  ...
  const selectedItemSummaries = selectedPolicies.map((policy) => ({
    sourceItemId: policy.officialSourceUrl?.match(/wlfareInfoId=([^&]+)/)?.[1] ?? null,
    title: policy.title,
    slug: policy.slug,
  ...
  const report = {
    ...
    skippedAlreadyAppliedCount: null,
    skippedCategoryMappingGapCount: null,
    compatibilityIssues,
    searchIndexPreview,
    pageGenerationPreview,
    guardValidation,
    ...
    applyReadiness,
    recommendedNextAction: applyReadiness === "ready_for_apply" ? "ready_for_bokjiro_central_apply" : "need_mapping_fix"
  };

  const candidateReport = await readJson(bokjiroPaths.promotionPreview).catch(() => null);
  if (candidateReport) {
    report.skippedDuplicateCount = candidateReport.skippedDuplicateCount;
    report.skippedAlreadyAppliedCount = candidateReport.skippedAlreadyAppliedCount;
    report.skippedCategoryMappingGapCount = candidateReport.skippedCategoryMappingGapCount;
  }
  ...
  console.log("Bokjiro central apply dry-run");
