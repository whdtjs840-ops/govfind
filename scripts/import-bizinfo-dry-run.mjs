...
import { bizinfoAdapter, BizinfoApiError } from "./importers/bizinfo-adapter.mjs";
import {
  BIZINFO_SOURCE_NAME,
  GOVFIND_CATEGORIES,
  GOVFIND_REGIONS,
  normalizeComparableText
} from "./importers/bizinfo-normalizers.mjs";
...
function isGenericOfficialUrl(value = "") {
  ...
  if (!url) return false;
  if (url === "https://www.bizinfo.go.kr") return true;
  return /^https:\/\/www\.bizinfo\.go\.kr\/?$/.test(url) || /\/web\/lay1\/bbs\/S1T122C128\/AS\/74\/list\.do/i.test(url);
}
...
function buildStagingItem({ rawIndex, listItem, detail, normalized, existingIndex }) {
  ...
  return {
    ...
    rawRef: { sourceName: BIZINFO_SOURCE_NAME, sourceItemId: normalized.sourceItemId, rawIndex },
    raw: { listItem, detail }
  };
}
...
async function saveArtifacts({ outDir, rawPayload, stagingPayload, report }) {
  ...
  const rawPath = resolve(outDir, "raw", `${BIZINFO_SOURCE_NAME}-${stamp}.raw.json`);
  const stagingPath = resolve(outDir, "staging", `${BIZINFO_SOURCE_NAME}-${stamp}.staging.json`);
  const reportPath = resolve(outDir, "reports", `${BIZINFO_SOURCE_NAME}-${stamp}.report.json`);
  const fixedReportPath = resolve("data/staging/bizinfo/dry-run-report.json");
...
function printReport(report) {
  console.log("Bizinfo dry-run importer");
...
export async function runBizinfoDryRun({ page = 1, limit = 50, save = false, outDir = "data/imports/bizinfo" } = {}) {
  const existingIndex = buildExistingPolicyIndex(getPublicPolicies(policies));

  if (!process.env.GOVFIND_BIZINFO_API_KEY && !process.env.GOVFIND_BIZINFO_CRTFC_KEY) {
    const rawPayload = {
      meta: {
        sourceName: BIZINFO_SOURCE_NAME,
    ...
    const stagingPayload = { sourceName: BIZINFO_SOURCE_NAME, fetchedAt: new Date().toISOString(), items: [] };
    const report = buildReport({
      rawPayload,
      stagingPayload,
      status: "error",
      error: { kind: "missing_api_key", message: "GOVFIND_BIZINFO_API_KEY is not set" }
    });
  ...
  try {
    ...
    const report = buildReport({
      ...
      status: "error",
      error:
        error instanceof BizinfoApiError
          ? error
...
async function main() {
  ...
  const limit = Number(argValue("limit", argValue("per-page", process.env.GOVFIND_BIZINFO_PER_PAGE ?? 50)));
  const save = hasFlag("save");
  const outDir = argValue("out-dir", "data/imports/bizinfo");
  const { report } = await runBizinfoDryRun({ page, limit, save, outDir });
  printReport(report);
